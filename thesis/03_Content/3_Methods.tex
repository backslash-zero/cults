...
\textbf{One corrected attribution record.} Hand-inspection of the interview archive's \texttt{attribution} field (the mechanism separating a participant's own words from the interviewer's, \ref{sec:vectorising_scholarly_work}) surfaced one further mislabel beyond the ones already documented there: document \texttt{b1-aug12-1231}, chunk 3 (\enquote{how it's perceived as a whole}) — the interviewer's own paraphrased question — had been tagged \texttt{attribution=participant} instead of \texttt{unspecified}. Corrected by hand, consistent with this project's existing practice for individually-identified mislabels of this kind. The shared space and interview-prototype layer were rebuilt after the correction; verified to produce byte-identical \texttt{shared\_space\_vector}s for all 44{,}520 points, since the fix touches only a metadata field, never an embedding. The geometric-analysis toolkit's current authoritative outputs are run \texttt{20260908-170432} (baseline, all 9 modules, unfiltered) and run \texttt{20260908-171742} (\texttt{analyze\_global\_structure}/\texttt{analyze\_cluster\_structure} only, restricted to \texttt{asserted}/\texttt{qualified} statements, as an epistemic-status robustness check) — both supersede the earlier \texttt{20260907-002832} run, which (along with \texttt{review\_pass1}/\texttt{smoketest5}/\texttt{test\_ie\_fast}) is kept on disk as superseded historical record rather than deleted.
